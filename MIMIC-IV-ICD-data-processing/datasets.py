...
def load_lookups(args, desc_embed=False):
    ...
    if args.Y == 'full':
        ...
        codes = set()
        # Determine which directory to use based on version
        if args.version == 'mimic4':
            codes_file = "%s/top_50/TOP_%s_CODES.csv" % (MIMIC_4_DIR, str(args.Y))
        else:
            codes_file = "%s/TOP_%s_CODES.csv" % (MIMIC_3_DIR, str(args.Y))
        ...
        desc_dict = load_code_descriptions(version=args.version)
...
def load_code_descriptions(version='mimic3'):
    ...
    if version == 'mimic2':
        ...
        # For MIMIC-IV with ICD-10 codes
        # Use code as description for now (you can add proper ICD-10 descriptions later)
        print("Note: Using simplified descriptions for MIMIC-IV ICD-10 codes")
        # The descriptions will be populated from the codes themselves
        # You can extend this if you have ICD-10 description files
        ...
        try:
            ...
            print("Warning: D_ICD_DIAGNOSES.csv not found")
        ...
        try:
            ...
            print("Warning: D_ICD_PROCEDURES.csv not found")
        ...
        try:
            ...
            print("Warning: ICD9_descriptions not found")
...
def load_description_vectors(Y, version='mimic3'):
    ...
    try:
        ...
        print(f"Warning: description_vectors.vocab not found in {data_dir}")
        print("Continuing without description embeddings (set --lmbda 0)")
